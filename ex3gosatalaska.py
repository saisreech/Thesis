...
#Can be used for multiple number of variables for GOSAT and places over the world by changing latitude and longitude values. The below is for  alaska satellite data 
...
def transferData(fileName):
		...
		df = df[(df.latitude >= 60) & (df.longitude >= -165)]
		df = df[(df.latitude <= 70) & (df.longitude <= -90)]
